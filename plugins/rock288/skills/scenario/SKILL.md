...
# rk:scenario — Edge Case & Scenario Explorer
...
| Next Step | Skill | How |
|-----------|-------|-----|
| Generate test cases from scenarios | `rk:test` | Pass scenario table as input context |
| Inform implementation plan risks | `rk:plan` | Paste Critical/High rows into risk assessment |
| Deep persona debate on top risks | `rk:predict` | Feed Critical scenarios as the change proposal |
...
```
/rk:scenario src/api/payment.ts
/rk:scenario "User registration with OAuth providers"
/rk:scenario src/middleware/auth.ts
/rk:scenario "Add multi-tenancy to the database layer"
```
