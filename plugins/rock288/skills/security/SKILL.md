---
name: security
description: "STRIDE + OWASP-based security audit with optional auto-fix. Scans code for vulnerabilities, categorizes by severity, and can iteratively fix findings using rk:autoresearch pattern."
...
# rk:security — Security Audit

Runs a structured STRIDE + OWASP security audit on a given scope. Produces a severity-ranked findings report. With `--fix`, applies fixes iteratively using the rk:autoresearch guard pattern.
...
|------|-----------|----------|
| Audit only | `/rk:security <scope>` | Scan → categorize → report |
| Audit + Fix | `/rk:security <scope> --fix` | Scan → categorize → fix iteratively |
| Bounded fix | `/rk:security <scope> --fix --iterations N` | Limit fix iterations to N |
...
4. Uses `rk:autoresearch` guard pattern for regression prevention
...
- Run after `rk:predict` when the security persona flags concerns
- Feed Critical/High findings into `rk:autoresearch --fix` for automated remediation
- Use `rk:scenario` with `--focus authorization` for deeper auth flow testing
- Pair with `rk:plan` to schedule Medium/Low findings as sprint tasks
...
/rk:security src/api/**/*.ts

# Audit entire src/ and auto-fix, max 15 iterations
/rk:security src/ --fix --iterations 15
