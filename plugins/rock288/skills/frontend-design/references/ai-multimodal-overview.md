...
Entry point for using the `rk:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `rk:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `rk:frontend-design` with `rk:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `rk:chrome-devtools` Skill
Use `rk:chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `rk:media-processing` Skill
