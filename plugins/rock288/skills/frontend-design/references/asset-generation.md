...
Complete workflow for generating design-aligned visual assets using `rk:ai-multimodal` skill.
...
Use `rk:ai-multimodal` skill's image generation capabilities:
...
Use `rk:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `rk:media-processing` skill for post-generation adjustments
