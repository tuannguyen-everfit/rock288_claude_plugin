...
Activate `rk:debug` skill. Use `debugger` subagent if needed.
...
|------|------------------|
| 1 | `rk:debug`, `debugger` subagent |
| 2 | Multiple `Explore` subagents in parallel (optional) |
| 3 | `rk:problem-solving`, `rk:sequential-thinking`, parallel `Bash` for verification |
| 4 | `tester` subagent |
