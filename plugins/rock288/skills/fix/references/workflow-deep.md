...
**Mandatory:** Activate `rk:scout` skill or launch 2-3 `Explore` subagents in parallel:
```
...
2. Activate `rk:debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `rk:sequential-thinking` — structured hypothesis formation.
4. Spawn parallel `Explore` subagents to test each hypothesis.
5. If 2+ hypotheses fail → auto-activate `rk:problem-solving`.
...
Implement per plan. Use `rk:context-engineering`, `rk:sequential-thinking`, `rk:problem-solving`.
...
- Activate `rk:project-management` for task sync-back, plan status updates, and progress tracking
...
|------|------------------|
| 1 | `rk:scout` OR parallel `Explore` subagents |
| 2 | `rk:debug`, `rk:sequential-thinking`, parallel `Explore`, (`rk:problem-solving` auto) |
| 3 | `researcher` (runs parallel with steps 1+2) |
| 4 | `rk:brainstorm` |
| 5 | `planner` |
| 6 | `rk:problem-solving`, `rk:sequential-thinking`, `rk:context-engineering` |
| 7 | `tester`, parallel `Bash` verification |
| 8 | `code-reviewer` |
| 9 | `rk:project-management`, `docs-manager`, `git-manager` |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `chrome`, `rk:chrome-devtools` or any relevant skills/tools to verify.
**Visual Assets:** Use `rk:ai-multimodal` for visual assets generation, analysis and verification.
