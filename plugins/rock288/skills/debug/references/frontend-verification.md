...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `rk:chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `rk:mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `rk:chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `rk:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
...
- Visual cherk: [pass | issues found]
