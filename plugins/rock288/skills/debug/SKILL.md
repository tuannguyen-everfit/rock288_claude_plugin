...
Visual verification of frontend implementations via Chrome MCP (Claude Chrome Extension) or `rk:chrome-devtools` skill fallback. Detect if frontend-related → check Chrome MCP availability → screenshot + console error check → report. Skip if not frontend.
...
- **Codebase:** `rk:docs-seeker` skill for package/plugin docs; `rk:repomix` skill for codebase summary
- **Scouting:** `/rk:scout` or `/rk:scout ext` for finding relevant files
- **Frontend:** Chrome browser or `rk:chrome-devtools` skill for visual verification (screenshots, console, network)
- **Skills:** Activate `rk:problem-solving` skill when stuck on complex issues
