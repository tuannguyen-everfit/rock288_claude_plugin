...
2. **Load other config** from `memory/ef_ship_slack.md` (per-repo). Expected fields:
   ...
   Flags `--slack-group`/`--slack-mentors` override individual fields for this run only (don't overwrite memory).
...
4. **Resolve mentors (every ship)** — 1–3 mentions, never auto-prefilled to last-used.
