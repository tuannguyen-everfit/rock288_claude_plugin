...
1. **Screenshots provided** → Use `rk:ai-multimodal` skill to analyze conversion issues
2. **Videos provided** → Use `rk:ai-multimodal` video-analysis for bottleneck identification
3. **URL provided** → Use `web_fetch` tool to fetch and analyze current issues
4. **Scout codebase** → `/rk:scout ext` (preferred) or `/rk:scout` to find relevant files
5. **Implement** → Use `fullstack-developer` agent to write enhanced copy into code files

## CRO Plan Creation Workflow

Use when creating a structured CRO optimization plan (e.g., via `/rk:plan --hard` with CRO-specific requirements).

### Steps

1. **Analyze** → Gather issues from screenshots/videos/URLs using steps above
2. **Activate** `rk:plan` skill for plan structure
