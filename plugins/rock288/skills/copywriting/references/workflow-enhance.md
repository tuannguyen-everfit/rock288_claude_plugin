...
1. **Screenshots provided** → Use `rk:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `rk:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/rk:scout ext` (preferred) or `/rk:scout` to find files needing enhancement
