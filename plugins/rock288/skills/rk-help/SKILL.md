...
- **`/rk:plan` → `/rk:cook`**: Best for high-risk or complex changes
- **`/rk:cook`**: Standalone for straightforward implementation
- **NEVER** claim `/rk:plan` is mandatory before `/rk:cook`
