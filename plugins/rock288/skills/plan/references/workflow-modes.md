...
4. **Context reminder:** `/rk:cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/rk:scout` to search codebase
...
7. **Context reminder:** `/rk:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `/rk:cook --parallel {absolute-plan-path}/plan.md`
...
```
Skill(skill: "rk:plan:red-team", args: "{plan-directory-path}")
```
...
| `auto` | Use the `Skill` tool: `Skill(skill: "rk:plan:validate", args: "{plan-directory-path}")` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Use the `Skill` tool to invoke `plan:validate`:
```
Skill(skill: "rk:plan:validate", args: "{plan-directory-path}")
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/rk:cook --auto {path}/plan.md` |
| hard | `/rk:cook {path}/plan.md` |
| parallel | `/rk:cook --parallel {path}/plan.md` |
| two | `/rk:cook {path}/plan.md` |
