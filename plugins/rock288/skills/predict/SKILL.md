...
# rk:predict — Multi-Persona Pre-Analysis
...
- Trivial or low-risk changes (use `rk:debug` for bugs, `rk:plan` for already-decided tasks)
...
|---------------|-------|-----|
| Deepen risk scenarios | `rk:scenario` | Feed Risk Summary rows as feature description |
| Create implementation plan | `rk:plan` | Attach Recommendations as constraints to planner |
| High-risk feature implementation | `rk:cook` | Reference CAUTION/STOP items as acceptance gates |
...
```
/rk:predict "Add WebSocket support for real-time notifications"
/rk:predict "Migrate authentication from JWT to session cookies"
/rk:predict "Add multi-tenancy to the database layer"
/rk:predict "Replace REST API with GraphQL" --files src/api/**/*.ts
```
