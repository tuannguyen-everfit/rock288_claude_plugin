 ...
 *
 * Fires when Plan subagent completes. Reminds to invoke /rk:cook --auto before implementation.
...
try {
  ...
  try {
    ...
    console.log('MUST invoke /rk:cook --auto skill before implementing the plan');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`Best Practice: Run /clear then /rk:cook ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Best Practice: Run /clear then /rk:cook {full-absolute-path-to-plan.md}');
    }
